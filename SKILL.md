...
description: 当面临多个候选方案需要选择、或需要"先内部推演再执行"的结构化决策任务时使用。典型场景：技术选型、架构设计、方案比较、问题排查、路径选择。Skill基于MCTS(蒙特卡洛树搜索)+TDL(时序差分学习)混合算法，每个候选方案在内部独立推演完整的因果链，汇总比较后选出最优方案再执行。
version: 1.3.0
license: MIT
---
