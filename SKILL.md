...
description: 通用决策引擎 — 每次收到用户消息，先启动发散引擎（多角度头脑风暴），强制生成至少3个候选方案，然后启动三引擎推演流程选出最优方案再执行。基于MCTS(蒙特卡洛树搜索)+TDL(时序差分学习)混合算法，每个方案独立推演完整因果链后汇总比较。适用于技术选型、架构设计、方案比较、问题排查等任何需要多方案评估的决策场景。
version: 1.3.0
license: MIT
alwaysApply: true
---
...
发散引擎的工作方式：
1. 从多个角度生成候选方案，**至少3个**，不设限制
2. 如果信息不足 → 追问用户 → 获得新信息后继续发散
3. 用约束条件过滤 → 收敛为可行方案

| 头脑风暴结果 | 行为 |
|-------------|------|
| 生成 ≥2 个可行方案 | ✅ 进入推演引擎，启动完整三引擎流程 |
| 所有方案都被约束过滤掉（只剩1个） | ❌ 直接执行，不启动推演 |
| 信息不足无法生成方案 | ⚡ 追问用户后重新发散 |

**关键：即使你觉得"这问题只有一种做法"，也先执行发散引擎试试——**
- "帮我看看这个文件" → 发散：读全部/搜关键词/只看结构/对比历史 → **至少有3种读法 → 启动推演**
- "把按钮改成蓝色" → 发散：改CSS变量/改行内样式/改主题配置/改JS动态 → **至少有3种改法 → 启动推演**
- "你好" → 发散：打招呼/问好/开启对话 → **但是所有做法结果一样 → 不启动**
- "什么是MCTS" → 发散：概述/对比/举例/数学推导 → **不同解释方式结果不同 → 启动推演**
