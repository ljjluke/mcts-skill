...
```
① 领域识别 → 八面审视: 构造8个具体维度（每面映射1个），自评0-10，识别盲区
② 八路侦查: 每面驱动1条侦查路径，盲区优先，学习深度门禁
③ 八面视角: 每面生成1个方案视角（力量/根基/变动/渗透/风险/显眼/边界/汇聚）

关键规则: 8面全用不遗漏、不凑数、不虚构、一需求一发散
```
...
| 发散引擎 | `engine/mcts-diverge.md` | 八面镜框架 + 八路侦查 + 八面视角 + 粗筛 |
